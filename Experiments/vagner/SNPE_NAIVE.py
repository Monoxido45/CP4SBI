...
theta_train = prior(num_samples=B_train).to(device)
X_train = simulator(theta_train).to(device)

thetas_calib = prior(num_samples=B_calib).to(device)
...
locart_coverage = np.zeros(num_coverage)
naive_coverage = np.zeros(num_coverage)
hdr_coverage = np.zeros(num_coverage)
global_coverage = np.zeros(num_coverage)
dict_keys = list(X_dict.keys())

for k in tqdm(range(num_coverage), desc="Computing coverage for each X observed"):
    X_0 = dict_keys[k]
    ...
    if score_type == "HPD":
        ...
        res = locart_conf.locart.sbi_score.compute(
            X_calib.cpu(),
            thetas_calib,
        )
        ...
        # testing also LOCART
        locart_cutoff = locart_conf.predict_cutoff(X_0)
        global_cutoff = global_conf.predict_cutoff(X_0)

        conf_scores = (
            -np.exp(
                post_estim.log_prob(
                    post_samples.to(device=device), x=X_0.to(device=device)
                ).cpu()
            )
            .cpu()
            .numpy()
        ...
        _, dens_samples = hdr_obj.recal_sample(
            y_hat=post_samples.cpu().reshape(
                1, post_samples.shape[0], post_samples.shape[1]
            ),
    ...
    naive_coverage[k] = np.mean(conf_scores <= closest_t)
    locart_coverage[k] = np.mean(conf_scores <= locart_cutoff)
    hdr_coverage[k] = np.mean(hdr_conf_scores <= hdr_cutoff)
    global_coverage[k] = np.mean(conf_scores <= global_cutoff)
...
prior, num_parameters, prior_returns_numpy = process_prior(
    prior_dist if task_name == "gaussian_linear" else prior
)
...
for i in range(num_test):
    ...
    if score_type == "HPD":
        ...
        conf_scores = (
            -np.exp(
                post_estim.log_prob(
                    post_samples.to(device=device), x=x_o.to(device=device)
                ).cpu()
            )
            .cpu()
            .numpy()
