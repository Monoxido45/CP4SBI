...
for file in files:
    ...
    new_index_array = np.delete(idx_array, 
                                best_method_index,
                                )
    ...
    lim_sup = (mae_array[-best_method_index] 
               + 
               se_array[-best_method_index]
               )
    
    lim_inf = (
        mae_array[best_method_index] 
        - 
        se_array[best_method_index]
        )
...
sorted_counts = dict(sorted(method_counts.items(), key=lambda item: item[1], reverse=True))
...
ax.set_xticklabels(
    [f"$\\bf{{{label}}}$" if label in ["LOCART", "CDF CP"] else label for label in sorted_counts.keys()]
)
